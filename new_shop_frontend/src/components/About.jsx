import React from 'react'
import { TbTruckReturn } from 'react-icons/tb'

const About = () => {
  
  return (
    <section className='max-padd-container py-12 xl:py-32'>
      {/* {container} */}
      <div className='flex flex-col gap-16 xl:gap-8 xl:flex-row'>
        {/* {left} */}
        <div className='flex-1'>
          <h3 className='h3 capitalize'>Unveiling Our Product's Key Featurues!</h3>
          <p className='py-5'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum voluptatem beatae provident nobis soluta nulla! Ea inventore reiciendis maxime maiores!</p>
        <div className='flex flex-col items-start gap-y-4'>
          
          <div className='flexCenter gap-x-4'>
            <div className='h-16 min-w-16 bg-secondary flexCenter rounded-md'>
              <TbTruckReturn className='text-white text-2xl'/>
            </div>
            <div>
              <h4 className='medium-18'>Easy Return Process</h4>
              <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Magnam, enim!</p>
            </div>
          </div>

          <div className='flexCenter gap-x-4'>
            <div className='h-16 min-w-16 bg-secondary flexCenter rounded-md'>
              <TbTruckReturn className='text-white text-2xl'/>
            </div>
            <div>
              <h4 className='medium-18'>Secure Payments Options</h4>
              <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Magnam, enim!</p>
            </div>
          </div>

          <div className='flexCenter gap-x-4'>
            <div className='h-16 min-w-16 bg-secondary flexCenter rounded-md'>
              <TbTruckReturn className='text-white text-2xl'/>
            </div>
            <div>
              <h4 className='medium-18'>Live Customer Support</h4>
              <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Magnam, enim!</p>
            </div>
          </div>

        </div>
        </div>
      </div>
      {/* {right} */}
      <div className='flex flexCenter'>
        <div>
          {/* <img src={} alt="" height={488} width={488}/> */}
        </div>
      </div>

    </section>
  )
}

export default About